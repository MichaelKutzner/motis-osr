...
struct bidirectional {
  ...
  void reset(Profile pr,
    ...
    distance_lon_degrees_ = geo::approx_distance_lng_degrees(
        std::abs(start_loc_.pos_.lat()) > std::abs(end_loc_.pos_.lat())
            ...
            : end_loc_.pos_);
    auto const diameter =
        pr.heuristic(distapprox(start_loc_.pos_, end_loc_.pos_));
    radius_ =
  ...
  void add(Profile pr,
    ...
    auto const heur = heuristic(pr, w, l.n_, dir, sharing);
  ...
  void add_start(Profile pr, ways const& w, label const l, sharing_data const* sharing) {
    if (kDebug) {
      l.get_node().print(std::cout, w);
      std::cout << "starting" << l.get_node().n_ << std::endl;
    }
    add(pr, w, l, direction::kForward, cost1_, pq1_, sharing);
  }

  void add_end(Profile pr, ways const& w, label const l, sharing_data const* sharing) {
    if (kDebug) {
      l.get_node().print(std::cout, w);
      std::cout << "ending" << l.get_node().n_ << std::endl;
    }
    add(pr, w, l, direction::kBackward, cost2_, pq2_, sharing);
  }
  ...
  double heuristic(Profile pr,
    ...
    return 0.5 * (pr.heuristic(dist) - pr.heuristic(other_dist)) *
  ...
  bool run_single(Profile pr,
    ...
    if (curr_cost < l.cost() - heuristic(pr, w, l.n_, SearchDir, sharing)) {
      return true;
    }
    if constexpr (kDebug) {
      std::cout << "EXTRACT ";
    ...
    pr.template adjacent<SearchDir, WithBlocked>(
        ...
        [&](node const neighbor, std::uint32_t const cost, distance_t,
          ...
          auto const heur =
              total + heuristic(pr, w, neighbor.n_, SearchDir, sharing);
    ...
    auto const handle_end_of_way_meetpoint = [&]() {
      ...
      if (opposite_candidate != end(*opposite_cost_map)) {
        ...
        if (other_cost != kInfeasible) {
          ...
          pr.template adjacent<opposite(SearchDir), WithBlocked>(
              r, curr, blocked, sharing, elevations,
  ...
  bool run(Profile pr,
    ...
    while (!pq1_.empty() || !pq2_.empty()) {
      if (!pq1_.empty() &&
          !run_single<SearchDir, WithBlocked>(pr, w, r, max, blocked, sharing,
                                              elevations, pq1_, cost1_)) {
        break;
      }
      if (!pq2_.empty() &&
          !run_single<opposite(SearchDir), WithBlocked>(
              pr, w, r, max, blocked, sharing, elevations, pq2_, cost2_)) {
        break;
      }
    }
  ...
  bool run(Profile pr,
    ...
    if (blocked == nullptr) {
      return dir == direction::kForward
                 ? run<direction::kForward, false>(pr, w, r, max, blocked, sharing,
                                                   elevations)
                 : run<direction::kBackward, false>(pr, w, r, max, blocked, sharing,
                                                    elevations);
    } else {
      return dir == direction::kForward
                 ? run<direction::kForward, true>(pr, w, r, max, blocked, sharing,
                                                  elevations)
                 : run<direction::kBackward, true>(pr, w, r, max, blocked, sharing,
                                                   elevations);
    }
  }
